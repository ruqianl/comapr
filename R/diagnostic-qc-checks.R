...
#' @param barcodeFile, defaults to NULL, it is assumed to be in the same directory as the 
...
perCellChrQC <- function(sampleName,chroms = c("chr1","chr7","chr15"),
  ...
  if(grepl("chr",segInfo_chrs$Chrom[1])){
    chr_levels <- paste0("chr",seq(1:19))
  } else {
    chr_levels <- paste0(seq(1:19))
  }
  
  plt_df <- segInfo_chrs %>% dplyr::group_by(ithSperm,Chrom) %>%
    dplyr::summarise(totalSNP = sum(nSNP),
                     nCORaw = length(nSNP)-1) %>% 
  ...
  if(doPlot){
    plt <- 
      ggplot(plt_df)+geom_point(mapping = aes(x = nCORaw,y = totalSNP))+
...
#' 
#' This function is used for checking whether chromosome segregation pattern obeys
#' the expected ratio.
#' 
#' @param chr, character, the chromosome to check
#' @param snpAnno, data.frame, the SNP annotation for the supplied chromosome
#' @param viState, dgTMatrix/Matrix, the viterbi state matrix, output from `sscocaller`
...
countBinState <- function(chr,
  ...
  tiles <- GenomicRanges::tileGenome(seqlengths(genomeRange)[chr],
                                     ntile = ntile)
  ...
  vi_bins <- suppressMessages(do.call(rbind,(apply(viState,2, function(x){
    data.frame(state = x, bins = hits@to) %>% dplyr::group_by(bins) %>% 
      dplyr::summarise(nstate2 = sum(state ==2),
                       nstate1 = sum(state ==1))
  }))))
  vi_bins %>% dplyr::mutate(binState = dplyr::if_else(nstate2>nstate1,
                                                      "state2",
                                                      "state1")) %>%
    dplyr::group_by(bins) %>% 
    dplyr::summarise(binSegRatio = sum(binState=="state2")/n(),
                     nstate2 =sum(binState=="state2"),
                     nstate1 = sum(binState=="state1")) %>%
...
perSegChrQC <- function(sampleName,chroms = c("chr1","chr7","chr15"),
  ...
  keepCells <- 
    segInfo_chrs %>% dplyr::group_by(ithSperm,Chrom) %>%
    dplyr::summarise(nCORaw = length(nSNP)-1) %>% 
    dplyr::filter(nCORaw < maxRawCO) %>%
    dplyr::select(ithSperm)
  
  p1 <-  segInfo_chrs %>% dplyr::filter(ithSperm %in% keepCells$ithSperm) %>%
    dplyr::mutate(bpDist=(Seg_end-Seg_start)) %>%
    ggplot() + geom_histogram(mapping = 
                                aes(x = nSNP))+scale_x_log10()+
    theme_classic()
  
  p2 <-  segInfo_chrs %>% dplyr::filter(ithSperm %in% keepCells$ithSperm) %>%
    dplyr::mutate(bpDist=(Seg_end-Seg_start)) %>%
    ggplot() + geom_histogram(mapping = 
                                aes(x = bpDist))+scale_x_log10()+
    theme_classic()
  
  p3 <-  segInfo_chrs %>% dplyr::filter(ithSperm %in% keepCells$ithSperm) %>%
    dplyr::mutate(bpDist=(Seg_end-Seg_start)) %>%
    ggplot() + geom_histogram(mapping = 
                                aes(x = logllRatio))+scale_x_log10()+
    theme_classic()
