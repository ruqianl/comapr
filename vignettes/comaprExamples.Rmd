...
```{r}
## only selecting the columns for samples
...
Next, we correct the geneotypes which includes 1), change genotype to labels, 2) infer the missing genotypes, 3) correct "Homo_ref" to "Het".
...
Then we plot the missing genotype again, we still have  missing values:

Marker `12_102311711` seems erroneous.
...
We need to remove the missing markers or samples with many missing values.
...
Filter samples or markers using `filterGT` by setting minimun markers for filtering samples and minimum samples for filtering markers:
...
We are not removing markers for this dataset since we do not have a lot markers.
...
```{r}
#saveRDS(gt_matrix_filtered, file="../data/")
...
If we set the type of the returned matrix as "counts", this function returns a matrix with cumulative crossover counts on each chromosome for each sample.
...
The `gt_matrix_co_by_marker` dataframe has a long format:
...
plotGTFreq(gt_matrix_filtered)+facet_wrap(.~variable)+
...
Then we call `calGeneticMap` which generates the point estimates of crossover rate, upper_ci, lower_ci and the mapped kosambi/haldane genetic lengths via defined mapping functions for each interval.
...
```
Two marker intervals were removed (9_83445613_92659805,9_92659805_94284389) because the crossover rates detected were larger than 0.5.
